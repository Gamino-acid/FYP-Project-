<?php
// ====================================================
// Student_view_result.php - UI Adapted to Mainpage
// ====================================================
include("connect.php");

$auth_user_id = $_GET['auth_user_id'] ?? null;
$current_page = 'view_result'; // 用于 Sidebar 高亮

if (!$auth_user_id) { header("location: login.php"); exit; }

// ====================================================
// 1. 获取用户信息 (逻辑同 Mainpage)
// ====================================================
$stud_data = [];
$user_name = 'Student';
...
<head>
    ...
    <style>
        ...
        .main-menu {
            ...
            position: fixed;
            top: 0; bottom: 0; height: 100%; left: 0; width: 60px;
        ...
        .result-header { text-align: center; margin-bottom: 30px; padding-bottom: 20px; border-bottom: 2px solid #f0f0f0; }
        .result-header h2 { margin: 0; color: #333; font-size: 22px; }
        
        .stud-info-box { display: flex; justify-content: space-between; margin-bottom: 30px; background: #f8f9fa; padding: 20px; border-radius: 8px; border: 1px solid #eee; }
        .stud-info-item strong { display: block; font-size: 12px; color: #666; text-transform: uppercase; margin-bottom: 4px; }
        .stud-info-item span { font-size: 15px; color: #333; font-weight: 600; }

        .score-table { width: 100%; border-collapse: collapse; margin-bottom: 30px; }
        .score-table th { background: #0056b3; color: #fff; text-align: left; padding: 12px 15px; font-size: 13px; text-transform: uppercase; }
        .score-table td { padding: 15px; border-bottom: 1px solid #eee; font-size: 14px; color: #444; }
        .score-table tr:last-child td { border-bottom: 2px solid #0056b3; }
        .total-row td { background: #e3effd; font-weight: 700; color: #0056b3; font-size: 16px; border-bottom: none; }

        .final-grade-box { text-align: center; margin-top: 30px; padding: 30px; border: 2px dashed #0056b3; border-radius: 12px; background: #f0f7ff; }
        .final-grade-display { font-size: 56px; font-weight: 800; color: #0056b3; line-height: 1; margin: 10px 0; }
        
        .pending-box { text-align: center; padding: 60px 20px; color: #888; border: 1px dashed #ccc; border-radius: 12px; }
        ...
        .badge-grp { background: #e3effd; color: #0056b3; }
        ...
        <div class="result-container">
            ...
            <?php if ($is_finalized): ?>
                ...
                <div class="final-grade-box">
                    <div style="color: #666; font-size: 14px; text-transform: uppercase; letter-spacing: 1px;">Overall Final Grade</div>
                    <div class="final-grade-display"><?php echo getGrade($total_score); ?></div>
                    <div style="font-weight: 600; color: #0056b3;"><?php echo ($total_score >= 50) ? "PASS" : "FAIL"; ?></div>
                </div>

                <div style="text-align: center; margin-top: 30px; font-size: 12px; color: #999; line-height:1.5;">
            ...
            <?php else: ?>
                <div class="pending-box">
                    ...
                    <div style="font-size: 18px; font-weight: 500; color: #555;">Results Not Yet Released</div>
