<?php
// ====================================================
// Supervisor_assignment_grade.php - 带总成绩单按钮版
// ====================================================
include("connect.php");
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if ($stmt = $conn->prepare($check_sql)) {
        ...
        echo "<script>alert('$msg'); window.location.href='Supervisor_assignment_grade.php?auth_user_id=" . urlencode($auth_user_id) . "&view_target=" . urlencode($redirect_target) . "&target_type=" . urlencode($redirect_type) . "';</script>";
    }
}
            ...
            <?php if ($view_mode == 'list_targets'): ?>
                ...
                <div class="card">
                    ...
                    <div class="grid-container">
                        ...
                        <?php endforeach; ?>
                        <?php if(empty($my_groups)) echo "<div style='color:#999; grid-column:1/-1;'>No groups assigned.</div>"; ?>
                    </div>
                </div>

                <div class="card">
                    <div class="card-header">My Individual Students</div>
                    <div class="grid-container">
                        ...
                        <?php endforeach; ?>
                        <?php if(empty($my_individuals)) echo "<div style='color:#999; grid-column:1/-1;'>No individual students assigned.</div>"; ?>
                    </div>
                </div>
                        ...
                        ?>
                            <div class="ass-row">
                                <div>
                                    ...
                                    <?php if($ass['fyp_marks'] > 0): ?>
                                        <div style="font-size:12px; color:var(--primary-color); margin-top:3px;"><strong>Score: <?php echo $ass['fyp_marks']; ?> / 100</strong></div>
                                    <?php endif; ?>
                                </div>
                                <div style="text-align:right;">
                                    ...
                                    <a href="?auth_user_id=<?php echo $auth_user_id; ?>&view_target=<?php echo $selected_target_id; ?>&target_type=<?php echo $selected_target_type; ?>&grade_assignment=<?php echo $ass['fyp_assignmentid']; ?>" 
                                       ...
                                       <?php echo ($st == 'Graded') ? 'Update Grade' : 'Grade Now'; ?> <i class="fa fa-chevron-right"></i>
                                    </a>
                                </div>
                            </div>
                        <?php endforeach; ?>
                    <?php else: ?>
                        <div style="padding:20px; text-align:center; color:#999;">No assignments assigned to this target.</div>
                    <?php endif; ?>
                </div>
                            ...
                            <form method="POST">
                                ...
                                <div style="display:flex; gap:15px; margin-bottom:10px;">
                                    <div style="flex:0 0 100px;">
                                        ...
                                        <input type="number" name="marks" value="<?php echo $stud['fyp_marks']; ?>" class="gf-input" min="0" max="100" placeholder="0-100">
                                    </div>
                                    <div style="flex:1;">
                                        <label style="font-size:12px; font-weight:600;">Feedback</label>
                                        <input type="text" name="feedback" value="<?php echo htmlspecialchars($stud['fyp_feedback']); ?>" class="gf-input" placeholder="Comments...">
                                    </div>
                                </div>
