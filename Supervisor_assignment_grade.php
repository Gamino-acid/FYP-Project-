<?php
// ====================================================
// Supervisor_assignment_grade.php - Mainpage Design + Pagination
// ====================================================
include("connect.php");
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $msg = isset($_POST['return_revision']) ? "Returned for Revision!" : "Grade Saved Successfully!";
    $icon = isset($_POST['return_revision']) ? "warning" : "success";
    ...
    if ($stmt = $conn->prepare($check_sql)) {
        ...
        $redirect_url = "Supervisor_assignment_grade.php?auth_user_id=" . urlencode($auth_user_id) . "&view_target=" . urlencode($redirect_target) . "&target_type=" . urlencode($redirect_type);
        
        // Construct SweetAlert
        $swal_script = "
        <script>
...
if ($sv_id) {
    if ($view_mode == 'list_targets') {
        ...
        $sql_groups = "SELECT DISTINCT g.group_id, g.group_name, 
                       ...
                       JOIN student_group g ON (s.fyp_studid = g.leader_id OR g.group_id IN (SELECT group_id FROM group_request WHERE invitee_id = s.fyp_studid AND request_status='Accepted'))
                       WHERE r.fyp_staffid = '$sv_id' AND s.fyp_group = 'Group'";
        ...
        while($row = $res_g->fetch_assoc()) {
            ...
            $sql_pending = "SELECT COUNT(*) as cnt FROM assignment a 
                            ...
                            AND (a.fyp_target_id = 'ALL' OR a.fyp_target_id = '$gid') 
                            AND a.fyp_assignment_type = 'Group'
                            AND (sub.fyp_submission_status = 'Turned In' OR sub.fyp_submission_status = 'Resubmitted')";
        ...
        $sql_indiv = "SELECT s.fyp_studid, s.fyp_studname
                      FROM fyp_registration r
                      JOIN student s ON r.fyp_studid = s.fyp_studid
                      WHERE r.fyp_staffid = '$sv_id' AND s.fyp_group = 'Individual'";
        ...
        while($row = $res_i->fetch_assoc()) {
             ...
             $sql_pending = "SELECT COUNT(*) as cnt FROM assignment a 
                            ...
                            WHERE a.fyp_staffid = '$sv_id' 
                            AND (a.fyp_target_id = 'ALL' OR a.fyp_target_id = '$sid') 
        ...
        $sql_ass = "SELECT a.*, 
                           sub.fyp_submission_status, sub.fyp_submission_date, sub.fyp_marks, sub.fyp_submitted_file
                    FROM assignment a
        ...
        if ($selected_target_type == 'Group') {
            ...
            $sql_members = "SELECT s.fyp_studid, s.fyp_studname, 
                                   sub.fyp_marks, sub.fyp_feedback, sub.fyp_submitted_file, sub.fyp_submission_status, sub.fyp_submission_date
                            FROM student s
        ...
        while($row = $res_m->fetch_assoc()) {
            ...
            $row['final_status'] = $row['fyp_submission_status'] ? $row['fyp_submission_status'] : 'Not Turned In';
             if (($row['final_status'] == 'Turned In') && $row['fyp_submission_date'] > $current_assignment['fyp_deadline']) {
                $row['final_status'] = 'Late Turned In';
            }
...
<body>
    ...
    <div class="main-content-wrapper">
        <div class="page-header">
            <div class="welcome-text"><h1>Grade Assignments</h1><p>Review and grade student submissions.</p></div>
            <div class="logo-section"><img src="image/ladybug.png" alt="Logo" class="logo-img"><span class="system-title">FYP Grading</span></div>
        ...
        <main class="main-content">
            <?php if ($view_mode == 'list_targets'): ?>
                <div style="display:flex; justify-content:space-between; align-items:center; margin-bottom:20px;">
                    ...
                    <a href="Supervisor_view_result.php?auth_user_id=<?php echo $auth_user_id; ?>" class="btn-action" style="background:#28a745; text-decoration:none;">
                    ...
                    <div class="list-container">
                        <?php foreach ($my_groups as $g): ?>
                            <a href="?auth_user_id=<?php echo $auth_user_id; ?>&view_target=<?php echo $g['group_id']; ?>&target_type=Group" class="list-row">
                                ...
                                <div class="row-meta">
                                    <?php if($g['pending_grading'] > 0): ?><span class="badge-pending"><?php echo $g['pending_grading']; ?> Pending</span><?php else: ?><span style="color:#28a745; font-size:12px;"><i class="fa fa-check"></i> All Graded</span><?php endif; ?>
                                    <div style="font-size:18px; color:#ccc; margin-top:5px;"><i class="fa fa-chevron-right"></i></div>
                                </div>
                            </a>
                        <?php endforeach; ?>
                        <?php if(empty($my_groups)) echo "<div style='color:#999; padding:15px;'>No groups assigned.</div>"; ?>
                    </div>
                </div>
                ...
                <div class="card">
                    <div class="card-header">
                        ...
                        <?php if(count($my_individuals) > 0) echo "<span style='font-size:12px; color:#666; font-weight:400;'>Max 15 per page</span>"; ?>
                    </div>
                    <div class="list-container">
                        <?php 
                            // Pagination Logic: Max 15 Students
                            $limit = 15;
                            $page = isset($_GET['page']) ? (int)$_GET['page'] : 1;
                            $total_students = count($my_individuals);
                            $total_pages = ceil($total_students / $limit);
                            $offset = ($page - 1) * $limit;
                            $display_students = array_slice($my_individuals, $offset, $limit);
                        
                            foreach ($display_students as $i): 
                        ?>
                            <a href="?auth_user_id=<?php echo $auth_user_id; ?>&view_target=<?php echo $i['fyp_studid']; ?>&target_type=Individual" class="list-row">
                                ...
                                <div class="row-info">
                                    <div class="row-title"><?php echo htmlspecialchars($i['fyp_studname']); ?></div>
                                    <div class="row-subtitle">ID: <?php echo $i['fyp_studid']; ?></div>
                                </div>
                                <div class="row-meta">
                                    <?php if($i['pending_grading'] > 0): ?><span class="badge-pending"><?php echo $i['pending_grading']; ?> Pending</span><?php else: ?><span style="color:#28a745; font-size:12px;"><i class="fa fa-check"></i> All Graded</span><?php endif; ?>
                                    <div style="font-size:18px; color:#ccc; margin-top:5px;"><i class="fa fa-chevron-right"></i></div>
                                </div>
                            </a>
                        <?php endforeach; ?>
                        
                        <?php if(empty($my_individuals)) echo "<div style='color:#999; padding:15px;'>No individual students assigned.</div>"; ?>
                    </div>
                    ...
                    <?php if($total_pages > 1): ?>
                        <div class="pagination">
                            <?php for($p = 1; $p <= $total_pages; $p++): ?>
                                <a href="?auth_user_id=<?php echo $auth_user_id; ?>&page=<?php echo $p; ?>" class="page-link <?php echo ($p == $page) ? 'active' : ''; ?>">
                                    <?php echo $p; ?>
                                </a>
                            <?php endfor; ?>
                        </div>
            ...
            <?php elseif ($view_mode == 'list_assignments'): ?>
                <a href="?auth_user_id=<?php echo $auth_user_id; ?>" class="back-btn"><i class="fa fa-arrow-left"></i> Back to Student List</a>
                <div class="card">
                    <div class="card-header">
                    ...
                    <?php if(count($target_assignments) > 0): ?>
                        <?php foreach($target_assignments as $ass): $st = $ass['final_status']; $css = str_replace(' ', '', $st); ?>
                            <div class="ass-row">
                                <div>
                                    ...
                                    <?php if($ass['fyp_marks'] > 0): ?><div style="font-size:12px; color:var(--primary-color); margin-top:3px;"><strong>Score: <?php echo $ass['fyp_marks']; ?> / 100</strong></div><?php endif; ?>
                                </div>
                                <div style="text-align:right;">
                                    <span class="status-pill st-<?php echo $css; ?>"><?php echo $st; ?></span><br>
                                    <a href="?auth_user_id=<?php echo $auth_user_id; ?>&view_target=<?php echo $selected_target_id; ?>&target_type=<?php echo $selected_target_type; ?>&grade_assignment=<?php echo $ass['fyp_assignmentid']; ?>" style="display:inline-block; margin-top:5px; font-size:12px; color:#0056b3;"><?php echo ($st == 'Graded') ? 'Update Grade' : 'Grade Now'; ?> <i class="fa fa-chevron-right"></i></a>
                                </div>
                            </div>
                        <?php endforeach; ?>
                    <?php else: ?><div style="padding:20px; text-align:center; color:#999;">No assignments assigned to this target.</div><?php endif; ?>
                </div>
                        ...
                        <div class="grading-box">
                            ...
                            <form method="POST">
                                ...
                                <div style="display:flex; gap:15px; margin-bottom:10px;">
                                    <div style="flex:0 0 100px;"><label style="font-size:12px; font-weight:600;">Marks</label><input type="number" name="marks" value="<?php echo $stud['fyp_marks']; ?>" class="gf-input" min="0" max="100" placeholder="0-100"></div>
                                    <div style="flex:1;"><label style="font-size:12px; font-weight:600;">Feedback</label><input type="text" name="feedback" value="<?php echo htmlspecialchars($stud['fyp_feedback']); ?>" class="gf-input" placeholder="Comments..."></div>
                                </div>
                                <button type="submit" name="save_grade" class="btn-action"><i class="fa fa-save"></i> Save Grade</button>
                                <button type="submit" name="return_revision" class="btn-action" style="background:#ffc107; color:black;"><i class="fa fa-undo"></i> Request Revision</button>
                            </form>
                        </div>
                    <?php endforeach; ?>
                </div>
