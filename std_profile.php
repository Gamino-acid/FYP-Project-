    ...
    if (isset($_POST['change_password'])) {
        ...
        if ($stmt = $conn->prepare($sql_check)) {
            ...
            if ($row = $res->fetch_assoc()) {
                // Assuming plain text comparison based on previous context. 
                // Ideally use password_verify($old_pass, $row['fyp_passwordhash'])
                if ($old_pass === $row['fyp_passwordhash']) {
                    $update_pass = "UPDATE USER SET fyp_passwordhash = ? WHERE fyp_userid = ?";
                    if ($stmt_up = $conn->prepare($update_pass)) {
                        $stmt_up->bind_param("si", $new_pass, $auth_user_id);
                        if ($stmt_up->execute()) {
...
<body>
    ...
    <div class="main-content-wrapper">
        ...
        <form id="profileForm" enctype="multipart/form-data">
            <div class="profile-container">
                <div class="left-column">
                    ...
                    <div style="font-size: 0.8em; color: var(--text-secondary); text-align: center;">(Student ID)</div>
                </div>
        ...
        <form id="passwordForm">
            <div class="profile-container" style="display:block;">
                ...
                <button type="submit" id="passBtn" class="save-btn" style="margin-top:10px;">
    ...
    <script>
        ...
        passwordForm.addEventListener('submit', async function(e) {
            ...
            passBtn.innerHTML = '<i class="fa fa-spinner fa-spin"></i> Updating...';
            ...
            try {
                ...
                passBtn.innerHTML = '<i class="fa fa-key"></i> Update Password';
            } catch (error) {
                showToast('An error occurred. Please try again.', 'error');
                passBtn.disabled = false;
                passBtn.innerHTML = '<i class="fa fa-key"></i> Update Password';
