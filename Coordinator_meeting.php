<?php
// ====================================================
// Coordinator_meeting.php - 日程管理 (Coordinator 版 - Staff ID & UI Updated)
// ====================================================
include("connect.php");
...
// 2. 获取 Coordinator 信息 (关键：获取 Staff ID)
$user_name = "Coordinator"; 
$user_avatar = "image/user.png"; 
$staff_id = ""; // 统一使用 Staff ID
$my_room = "Office"; 
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (isset($_POST['add_slot'])) {
        if (empty($staff_id)) {
            echo "<script>alert('Error: Staff ID not found for this Coordinator profile.');</script>";
        } else {
            ...
            if ($start_time >= $end_time) {
                ...
                if ($stmt = $conn->prepare($sql_add)) {
                    ...
                    if ($count_added > 0) {
                        echo "<script>alert('Success! $count_added slots added.'); window.location.href='Coordinator_meeting.php?auth_user_id=" . urlencode($auth_user_id) . "';</script>";
                    } else {
    ...
    if (isset($_POST['edit_slot'])) {
        ...
        if ($new_start_time >= $new_end_time) {
            ...
            if ($stmt = $conn->prepare($sql_edit)) {
                ...
                if ($stmt->execute()) {
                    echo "<script>alert('Slot updated successfully!'); window.location.href='Coordinator_meeting.php?auth_user_id=" . urlencode($auth_user_id) . "';</script>";
                } else {
    ...
    if (isset($_POST['delete_slot'])) {
        ...
        echo "<script>window.location.href='Coordinator_meeting.php?auth_user_id=" . urlencode($auth_user_id) . "';</script>";
    }
    
    // D. 处理预约请求 (Approve/Reject/Cancel)
    if (isset($_POST['handle_appointment'])) {
        ...
        if ($action == 'approve') {
            ...
            echo "<script>alert('Appointment Approved!'); window.location.href='Coordinator_meeting.php?auth_user_id=" . urlencode($auth_user_id) . "';</script>";
        } 
        elseif ($action == 'reject' || $action == 'cancel') {
            ...
            echo "<script>alert('$msg'); window.location.href='Coordinator_meeting.php?auth_user_id=" . urlencode($auth_user_id) . "';</script>";
        }
    }
}

// ====================================================
// 4. 获取数据 (GET) - 使用 fyp_staffid 查询
// ====================================================
...
if (!empty($staff_id)) {
    ...
    // C. Available Slots (使用 fyp_staffid)
