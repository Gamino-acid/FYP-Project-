...
<head>
    ...
    <style>
        ...
        .info-card { background: #fff; padding: 25px; border-radius: 12px; box-shadow: var(--card-shadow); display: flex; flex-direction: column; transition: transform 0.2s; border-left: 4px solid var(--primary-color); }
        ...
        .slot-card { background: #fff; border: 1px solid #e0e0e0; border-radius: 8px; padding: 15px; text-align: center; transition: all 0.3s; cursor: pointer; }
        ...
        .ann-card { background: #fff; border-radius: 8px; padding: 20px; margin-bottom: 15px; box-shadow: var(--card-shadow); border-left: 5px solid #6264A7; }
        .ann-header { display: flex; justify-content: space-between; margin-bottom: 12px; padding-bottom: 12px; border-bottom: 1px solid #eee; }
        ...
        .modal-box { background: #fff; padding: 30px; border-radius: 12px; width: 90%; max-width: 400px; text-align: center; box-shadow: 0 10px 30px rgba(0,0,0,0.2); }
        ...
        .empty-state { text-align: center; padding: 40px; color: #999; background: white; border-radius: 12px; box-shadow: var(--card-shadow); }

        .pagination-container { display: flex; justify-content: space-between; align-items: center; margin-top: 20px; }
        .page-btn { padding: 8px 15px; background: white; border: 1px solid #ddd; border-radius: 6px; text-decoration: none; color: #333; transition: 0.2s; }
        .page-btn:hover:not(.disabled) { background: var(--primary-color); color: white; border-color: var(--primary-color); }
        .page-btn.disabled { opacity: 0.5; cursor: not-allowed; }

        /* Topbar specific logo style */
        .logo-section {
            display: flex;
            align-items: center;
            gap: 12px;
...
<body>
    ...
    <div class="main-content-wrapper">
        ...
        <?php if ($current_page == 'dashboard'): ?>
            <div class="info-cards-grid">
                <div class="info-card">
                    ...
                    <p style="font-size:15px; color:#555;"><?php echo htmlspecialchars($dashboard_data['project_status']); ?></p>
                </div>
                <div class="info-card" style="border-left-color: #d93025;">
                    <h3 style="color: #d93025;"><i class="fa fa-clock-o"></i> Next Deadline</h3>
                    <p style="font-size:15px; color:#555;"><?php echo htmlspecialchars($dashboard_data['next_deadline']); ?></p>
                </div>
                <div class="info-card" style="border-left-color: #28a745;">
                    <h3 style="color: #28a745;"><i class="fa fa-user"></i> Supervisor</h3>
                    <p style="font-size:15px; color:#555;"><?php echo htmlspecialchars($dashboard_data['supervisor_name']); ?></p>
                </div>
            </div>

            <h3 style="margin-bottom: 20px; color:#333;">Latest Announcements</h3>
            ...
            <?php if (count($display_announcements) > 0): ?>
                    ...
                    <div class="ann-card">
                        <div class="ann-header">
                            <div class="ann-sender-info">
                                ...
                                <div>
                                    <div style="font-weight:600; color:#333;"><?php echo htmlspecialchars($ann['final_display_name']); ?> </div>
                                    <div style="font-size: 11px; color: #888;">To: <?php echo htmlspecialchars($ann['fyp_receiver']); ?></div>
                                </div>
                            </div>
                            <div style="font-size: 12px; color: #999;"><i class="fa fa-clock-o"></i> <?php $date = date_create($ann['fyp_datecreated']); echo date_format($date, "M d, Y h:i A"); ?></div>
                        </div>
                        <div style="font-size: 18px; font-weight: 600; color: #2c2c2c; margin-bottom: 8px;"><?php echo htmlspecialchars($ann['fyp_subject']); ?></div>
                        <div style="color: #555; line-height: 1.6; font-size: 14px;"><?php echo nl2br(htmlspecialchars($ann['fyp_description'])); ?></div>
                    </div>
                <?php endforeach; ?>
                ...
                <?php if ($total_pages > 1): ?>
                    <div class="pagination-container">
                        ...
                        <span style="font-size: 13px; color: #888;">Page <?php echo $current_ann_page; ?> of <?php echo $total_pages; ?></span>
        ...
        <?php elseif ($current_page == 'appointments'): ?>
            ...
            <?php else: ?>
                <div class="supervisor-profile-card" style="background:white; padding:25px; border-radius:12px; box-shadow:0 4px 6px rgba(0,0,0,0.05); display:flex; align-items:center; gap:20px; margin-bottom:30px; border-left:5px solid #28a745;">
                    ...
                    <div>
                        <h2 style="margin:0 0 5px 0; font-size:20px; color:#333;"><?php echo htmlspecialchars($supervisor_data['fyp_name']); ?></h2>
                        <p style="margin:3px 0; color:#666; font-size:14px;"><i class="fa fa-envelope" style="width:20px;"></i> <?php echo htmlspecialchars($supervisor_data['fyp_email']); ?></p>
                        <p style="margin:3px 0; color:#666; font-size:14px;"><i class="fa fa-phone" style="width:20px;"></i> <?php echo htmlspecialchars($supervisor_data['fyp_contactno']); ?></p>
                    </div>
                </div>

                <h3 style="margin-bottom:15px; color:#333;">Available Slots</h3>
                    ...
                    <div class="slots-grid">
                        <?php foreach ($available_schedules as $sch): ?>
                            <div class="slot-card" onclick="openBookModal('<?php echo $sch['fyp_scheduleid']; ?>', '<?php echo $sch['fyp_day'] . ', ' . $sch['fyp_date'] . ' (' . $sch['fyp_fromtime'] . ')'; ?>', '<?php echo $supervisor_data['fyp_supervisorid']; ?>', '<?php echo $pairing_data['fyp_pairingid']; ?>', '<?php echo $stud_data['fyp_studid']; ?>')">
                                ...
                                <div style="font-size:13px; color:#777; margin-bottom:10px;"><?php echo htmlspecialchars($sch['fyp_date']); ?></div>
                                <div style="font-size:15px; font-weight:500; color:#333; background:#f8f9fa; padding:5px; border-radius:4px;">
                ...
                <?php else: ?>
                    <p style="color:#777;">No available slots found for this supervisor at the moment.</p>
                <?php endif; ?>
            <?php endif; ?>
    ...
    <div id="confirmationModal" class="modal-overlay">
        <div class="modal-box">
            ...
            <div style="font-size:20px; font-weight:600; margin-bottom:10px; color:#333;">Confirm Action</div>
            <p style="color:#666; margin-bottom:30px; font-size:15px; line-height:1.6;" id="modalText"></p>
